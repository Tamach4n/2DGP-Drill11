...
class Idle:
    ...
    def do(boy):
        boy.frame = (
            boy.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time
        ) % 8
    ...
    def draw(boy):
        boy.image.clip_draw(
            int(boy.frame) * 100, boy.action * 100, 100, 100, boy.x, boy.y
        )
...
class Boy:
    ...
    def get_bb(self):  #   Bounding Box
        if self.state_machine.cur_state == Sleep:
            if self.face_dir == 1:
                return self.x - 80, self.y - 40, self.x + 20, self.y
